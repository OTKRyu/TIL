...
- 함수 
  ...
  - 얼마나 들어올지 알 수 없는 인자에는 `...argnames` 이런식으로 적어준다. 그리고 이 `...`은 파이썬의 `*`와 같이 리스트 요소 하나하나를 분해한 것을 의미하기도 한다. 그 외에도 object에서 키와 밸류값을 하나하나씩 풀어놓는 것도 가능하다.
  
  - `function funcname(arg) { return value}` : 기본적으로 함수를 생성하는 법, 선언식.예전부터 쓰던 것이기 때문에 var객체로 함수가 생성된다.
...
- lodash
  - 다양하고 잡다한 프로그래밍 도구들을 모아놓은 라이브러리
  - 수학 관련부터 유니코드 인코딩관련 툴까지 이것저것 많다.
